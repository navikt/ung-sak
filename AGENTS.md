...
- **Opphør og opphevelse i samme vs. ulik behandling**: Avgjørende skille er om opphøret ble vedtatt i en tidligere, avsluttet behandling (`OPPHØR_OPPHEVET`, gir eget vedtaksbrev), eller om opphør og opphevelse havnet på **samme, fortsatt åpne** behandling (`OPPHØR_MOTTATT_OG_ANNULLERT_I_SAMME_BEHANDLING`, ingen brev — det finnes ikke noe opphørsvedtak å reversere). Delt utleder: `OpphørOpphevetUtleder.opphørVarFaktiskIverksatt(Behandling, UngdomsprogramPeriodeRepository)` (i `behandlingslager`) sjekker om originalbehandlingen hadde lukket sluttdato (ikke `TIDENES_ENDE`). Brukes av tre konsumenter: (1) `OpphørOpphevetStrategy` (brevutsendelse, se punktet over), (2) `BehandlingDtoUtil.utledVisningsnavn` (velger `BehandlingVisningsnavn.OPPHØR_OPPHEVET` vs. `OPPHØR_MOTTATT_OG_ANNULLERT_I_SAMME_BEHANDLING`), (3) `UtledStatusForPerioderPåBehandling.mapTilÅrsakTilVurdering` (velger `ÅrsakTilVurdering.OPPHØR_OPPHEVET_UNGDOMSPROGRAM` vs. `OPPHØR_MOTTATT_OG_ANNULLERT_I_SAMME_BEHANDLING_UNGDOMSPROGRAM` i periodevisningen), kun for tilfeller der behandlingsårsaken er `RE_HENDELSE_OPPHØR_OPPHEVET_UNGDOMSPROGRAM`. `ÅrsakTilVurdering.SAMMENHENG` (statisk 1:1-mapping `BehandlingÅrsakType`→`ÅrsakTilVurdering`) beholder default-mappingen til `OPPHØR_OPPHEVET_UNGDOMSPROGRAM`; overstyringen til `OPPHØR_MOTTATT_OG_ANNULLERT_I_SAMME_BEHANDLING_UNGDOMSPROGRAM` håndteres kontekstuelt i `mapTilÅrsakTilVurdering`, ikke i det statiske kartet.

...
- Brev/kodeverk: `DokumentMalType.OPPHOR_OPPHEVET_DOK`, `TemplateType.OPPHOR_OPPHEVET`, mal `opphør_opphevet.hbs`, `OpphørOpphevetStrategy`/`OpphørOpphevetInnholdBygger`/`OpphørOpphevetDto` (viser tidligere sluttdato og gjeldende maksdato), `DetaljertResultatType.OPPHØR_OPPHEVET`, `BehandlingVisningsnavn.UNGDOMSPROGRAM_OPPHØR_OPPHEVET`, `ÅrsakTilVurdering.UNGDOMSPROGRAM_OPPHØR_OPPHEVET`.

...
- **«Opphevet» vs. «Annullert» — begrepsforskjell**: «Opphevet» brukes når opphøret faktisk ble vedtatt/iverksatt (reelt opphørsbrev sendt) og deretter reverseres av en ny hendelse — bruker får da et eget «Opphør opphevet»-vedtaksbrev. «Annullert» brukes når opphøret **aldri** rakk å bli vedtatt (f.eks. fordi opphevelseshendelsen kom inn mens den opprinnelige opphør-behandlingen fortsatt ventet på uttalelse fra deltaker) — da er det ingenting å oppheve, kun å avbryte/annullere den ubesvarte opphør-behandlingen. Skillet er sentralt: «opphevet» impliserer et brev til bruker, «annullert» impliserer at ingen brev noensinne ble/blir sendt om opphør. Delt utleder: `OpphørOpphevetUtleder.opphørVarFaktiskIverksatt(Behandling, UngdomsprogramPeriodeRepository)` (i `behandlingslager`) er eneste kilde til sannhet for dette skillet — sjekker om originalbehandlingen hadde lukket sluttdato (ikke `TIDENES_ENDE`). Brukes av tre konsumenter: (1) `OpphørOpphevetStrategy` (brevutsendelse, se punktet over), (2) `BehandlingDtoUtil.utledVisningsnavn` (velger `BehandlingVisningsnavn.OPPHØR_OPPHEVET` vs. `OPPHØR_ANNULERT`), (3) `UtledStatusForPerioderPåBehandling.mapTilÅrsakTilVurdering` (velger `ÅrsakTilVurdering.OPPHØR_OPPHEVET_UNGDOMSPROGRAM` vs. `OPPHØR_ANNULERT_UNGDOMSPROGRAM` i periodevisningen), kun for tilfeller der behandlingsårsaken er `RE_HENDELSE_OPPHØR_OPPHEVET_UNGDOMSPROGRAM`. `ÅrsakTilVurdering.SAMMENHENG` (statisk 1:1-mapping `BehandlingÅrsakType`→`ÅrsakTilVurdering`) beholder default-mappingen til `OPPHØR_OPPHEVET_UNGDOMSPROGRAM`; annullert-overstyringen håndteres kontekstuelt i `mapTilÅrsakTilVurdering`, ikke i det statiske kartet.

 ...
 */
@ApplicationScoped
@ProsessTask(OpprettKontrollBehandlingEllerDiffTask.TASKNAME)
// gruppeSekvens = false for å kunne hoppe tilbake ved feilende fortsettBehandling task
@FagsakProsesstaskRekkefølge(gruppeSekvens = true)
public class OpprettKontrollBehandlingEllerDiffTask extends FagsakProsessTask {

    public static final String TASKNAME = "behandlingskontroll.opprettKontrollEllerDiff";
    ...
    private static final Logger log = LoggerFactory.getLogger(OpprettKontrollBehandlingEllerDiffTask.class);
    ...
    OpprettKontrollBehandlingEllerDiffTask() {
        // for CDI proxy
    }

    @Inject
    public OpprettKontrollBehandlingEllerDiffTask(FagsakRepository fagsakRepository,
    ...
    protected void prosesser(ProsessTaskData prosessTaskData) {
        ...
        var behandlingIder = behandlingRepository.hentÅpneBehandlingerForFagsakId(fagsakId, BehandlingType.KONTROLLBEHANDLING).stream().map(Behandling::getId).toList();
        ...
        if (behandlingIder.isEmpty()) {
            ...
            final KontrollBehandlingTjeneste revurderingTjeneste = FagsakYtelseTypeRef.Lookup.find(KontrollBehandlingTjeneste.class, fagsak.getYtelseType()).orElseThrow();
            if (sisteVedtak.isPresent()) {
                var origBehandling = sisteVedtak.get();
                var behandling = revurderingTjeneste.opprettKontrollbehandling(origBehandling, behandlingÅrsakType, origBehandling.getBehandlendeOrganisasjonsEnhet());
            ...
            if (behandlingIder.size() != 1) {
                throw new IllegalStateException("Fant flere åpne behandlinger");
            }
            var behandlingId = behandlingIder.getFirst();
            ...
            behandlingProsesseringTjeneste.opprettTasksForGjenopptaOppdaterFortsett(behandling, false, false);

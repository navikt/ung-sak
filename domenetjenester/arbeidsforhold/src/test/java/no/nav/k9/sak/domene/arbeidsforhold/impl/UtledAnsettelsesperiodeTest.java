...
import no.nav.k9.sak.domene.arbeidsforhold.impl.UtledAnsettelsesperiode;
...
public class UtledAnsettelsesperiodeTest {
    ...
    public void skal_finne_korrekt_periode_for_yrkesaktiviteter_med_flere_ansettelsesperioder_hvor_en_overlapper_stp(){
        ...
        // Act
        Optional<DatoIntervallEntitet> ansettelsesperiode = UtledAnsettelsesperiode.utled(new YrkesaktivitetFilter(Optional.empty(), yrkesaktivitet), yrkesaktivitet, SKJÆRINGDSTIDSPUNKT, false);
    ...
    public void skal_finne_ansettelsesperiode_som_overlapper_stp_og_har_aktivitetsavtale_med_fom_dato_nærmest_stp(){
        ...
        Optional<DatoIntervallEntitet> ansettelsesperiode = UtledAnsettelsesperiode.utled(new YrkesaktivitetFilter(null, yrkesaktiviteter), yrkesaktiviteter, SKJÆRINGDSTIDSPUNKT, false);
        // Assert
        assertThat(ansettelsesperiode).hasValueSatisfying(ap -> {
            assertThat(ap.getFomDato()).isEqualTo(SKJÆRINGDSTIDSPUNKT.minusYears(1));
            assertThat(ap.getTomDato()).isEqualTo(Tid.TIDENES_ENDE);
    ...
    public void skal_finne_overstyrt_ansettelsesperiode_når_perioden_overlapper_stp(){
        ...
        var filter = new YrkesaktivitetFilter(grunnlag.getArbeidsforholdInformasjon(), grunnlag.getAktørArbeidFraRegister(behandling.getAktørId())).før(SKJÆRINGDSTIDSPUNKT);
    ...
    private InntektArbeidYtelseGrunnlag lagGrunnlag(InntektArbeidYtelseAggregatBuilder.AktørArbeidBuilder aktørArbeidBuilder, Optional<ArbeidsforholdInformasjon> arbeidsforholdInformasjonOpt) {

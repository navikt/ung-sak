package no.nav.k9.sak.mottak.dokumentmottak.impl;

public class HåndterMottattDokumentTaskProperties {
    public static final String TASKTYPE = HåndterMottattDokumentTask.TASKTYPE;

    public static final String BEHANDLING_ÅRSAK_TYPE_KEY = "arsakType";
    public static final String MOTTATT_DOKUMENT_ID_KEY = "mottattDokumentId";

    HåndterMottattDokumentTaskProperties() {
        // Skal ikke konstrueres
    }
}

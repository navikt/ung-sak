...
@ApplicationScoped
@ProsessTask(value = RevurderingMetrikkManuellTask.TASKTYPE)
public class RevurderingMetrikkManuellTask implements ProsessTaskHandler {

    private static final int CHUNK_EVENT_SIZE = 1000;

    private static final int LOG_THRESHOLD = 5000;

    static final String TASKTYPE = "revurdering.metrikk.manuell.task";

    private static final Logger log = LoggerFactory.getLogger(RevurderingMetrikkManuellTask.class);

    private SensuKlient sensuKlient;

    private RevurderingMetrikkRepository revurderingMetrikkRepository;

    RevurderingMetrikkManuellTask() {
        // for proxyd
    }

    @Inject
    public RevurderingMetrikkManuellTask(SensuKlient sensuKlient, RevurderingMetrikkRepository revurderingMetrikkRepository) {

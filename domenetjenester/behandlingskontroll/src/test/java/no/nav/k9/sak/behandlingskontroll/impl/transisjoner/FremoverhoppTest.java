...
public class FremoverhoppTest {
    ...
    public void skal_kalle_transisjoner_på_steg_det_hoppes_over() throws Exception {
        assertThat(transisjonerVedFremoverhopp(fra(steg1, INN), til(steg3))).contains(
            no.nav.k9.sak.behandlingskontroll.impl.observer.StegTransisjon.hoppFremoverOver(steg1),
            no.nav.k9.sak.behandlingskontroll.impl.observer.StegTransisjon.hoppFremoverOver(steg2));
        assertThat(transisjonerVedFremoverhopp(fra(steg1, UT), til(steg3)))
            .contains(no.nav.k9.sak.behandlingskontroll.impl.observer.StegTransisjon.hoppFremoverOver(steg2));
    ...
    private List<no.nav.k9.sak.behandlingskontroll.impl.observer.StegTransisjon> transisjonerVedFremoverhopp(StegPort fra, BehandlingStegType til) {
    ...
    class TestSteg implements BehandlingSteg {
        ...
        public void vedHoppOverFramover(BehandlingskontrollKontekst kontekst, BehandlingStegModell modell, BehandlingStegType fraSteg,
            ...
            transisjoner.add(new no.nav.k9.sak.behandlingskontroll.impl.observer.StegTransisjon(TransisjonType.HOPP_OVER_FRAMOVER, behandlingStegType));
        }

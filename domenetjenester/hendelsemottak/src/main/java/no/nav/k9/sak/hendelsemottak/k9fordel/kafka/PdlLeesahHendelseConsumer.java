...
@ApplicationScoped
public class PdlLeesahHendelseConsumer implements AppServiceHandler {
    ...
    @Inject
    public PdlLeesahHendelseConsumer(PdlLeesahHendelseHåndterer pdlLeesahHendelseHåndterer,
                                     PdlLeesahHendelseProperties streamKafkaProperties) {
        ...
        Consumed<String, Personhendelse> consumed = Consumed.<String, Personhendelse>with(Topology.AutoOffsetReset.LATEST) // TODO: Endre til NONE etter prodsetting
            .withKeySerde(topic.getSerdeKey())
            .withValueSerde(topic.getSerdeValue());

        builder.stream(topic.getTopic(), consumed)
            .foreach((key, payload) -> pdlLeesahHendelseHåndterer.handleMessage(payload));

        Topology topology = builder.build();
        stream = new KafkaStreams(topology, props);
    }

    private Properties setupProperties(PdlLeesahHendelseProperties streamProperties) {
...
/*        if (streamProperties.getSchemaRegistryUrl() != null) {
        ...
        if (streamProperties.getSchemaRegistryUrl() != null && !streamProperties.getSchemaRegistryUrl().isEmpty()) {
            var schemaMap = Map.of("schema.registry.url", streamProperties.getSchemaRegistryUrl(), "specific.avro.reader", true);

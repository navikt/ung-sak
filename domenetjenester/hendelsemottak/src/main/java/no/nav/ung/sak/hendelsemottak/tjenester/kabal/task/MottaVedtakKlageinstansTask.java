...
public class MottaVedtakKlageinstansTask implements ProsessTaskHandler {
    ...
    public void doTask(ProsessTaskData prosessTaskData) {
        ...
        if (prosessTaskData.getPropertyValue(KABAL_EVENTTYPE).equals(KabalBehandlingEvent.Eventtyper.ANKEBEHANDLING_AVSLUTTET)) {
            ...
            var klagevurderingNK = klageUtredning.hentKlagevurdering(KlageVurdertAv.NK_KABAL);
            if (klagevurderingNK.isPresent()) {
                throw new IllegalStateException("Forsokte oppdatere behandlingsresultat fra kabal men behandlingsresultat var allerede satt till: "
                    + klageUtredning.getKlageVurderingType(KlageVurdertAv.NK_KABAL).map(KlageVurderingType::getNavn).get());
            }
    ...
    private void opprettGjenopptaBehandlingTask(no.nav.ung.sak.behandlingslager.behandling.Behandling behandling) {
        ProsessTaskData gjenopptaBehandlingTask = ProsessTaskData.forProsessTask(GjenopptaBehandlingTask.class);
        gjenopptaBehandlingTask.setBehandling(behandling.getFagsakId(), behandling.getId(), behandling.getAktørId().getId());
        gjenopptaBehandlingTask.setSekvens("1");
        gjenopptaBehandlingTask.setPrioritet(100);

        gjenopptaBehandlingTask.setCallIdFraEksisterende();

        prosessTaskRepository.lagre(gjenopptaBehandlingTask);
    }

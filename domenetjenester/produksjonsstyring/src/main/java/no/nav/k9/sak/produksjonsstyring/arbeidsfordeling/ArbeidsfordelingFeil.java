...
public interface ArbeidsfordelingFeil extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "FP-569566", feilmelding = "Finner ikke behandlende enhet for geografisk tilknytning '%s', diskresjonskode '%s', behandlingstema '%s'", logLevel = WARN)
    Feil finnerIkkeBehandlendeEnhet(String geografiskTilknytning, String diskresjonskode, BehandlingTema behandlingTema);

    @TekniskFeil(feilkode = "FP-004703", feilmelding = "Forventet en, men fikk flere alternative behandlende enheter for geografisk tilknytning '%s', diskresjonskode '%s', behandlingstema  '%s': '%s'. Valgte '%s'", logLevel = WARN)

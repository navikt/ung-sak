...
import static no.nav.folketrygdloven.beregningsgrunnlag.regelmodell.regelmodell.AktivitetStatus.FL;
...
@RuleDocumentation(FastsettSammenligningsgrunnlagFL.ID)
class FastsettSammenligningsgrunnlagFL extends LeafSpecification<BeregningsgrunnlagPeriode> {

    static final String ID = "FP_BR 28.2";
    static final String BESKRIVELSE = "Sammenligningsgrunnlag for FL er sum av inntektene i sammenligningsperioden";

    FastsettSammenligningsgrunnlagFL() {
        super(ID, BESKRIVELSE);
    }

    @Override
    public Evaluation evaluate(BeregningsgrunnlagPeriode grunnlag) {
        if (grunnlag.getSammenligningsgrunnlagPrStatus(AktivitetStatus.FL) == null) {
            Periode sammenligningsPeriode = lagSammenligningsPeriode(grunnlag.getInntektsgrunnlag(), grunnlag.getSkjæringstidspunkt());
            BigDecimal sammenligningsgrunnlagInntekt = grunnlag.getInntektsgrunnlag().getSamletInntektISammenligningsperiodeFL(sammenligningsPeriode);
            SammenligningsGrunnlag sg = SammenligningsGrunnlag.builder()
                ...
                .build();
            Beregningsgrunnlag.builder(grunnlag.getBeregningsgrunnlag()).medSammenligningsgrunnlagPrStatus(FL, sg).build();
        }

        Map<String, Object> resultater = new HashMap<>();
        SammenligningsGrunnlag sammenligningsGrunnlag = grunnlag.getSammenligningsgrunnlagPrStatus(FL);

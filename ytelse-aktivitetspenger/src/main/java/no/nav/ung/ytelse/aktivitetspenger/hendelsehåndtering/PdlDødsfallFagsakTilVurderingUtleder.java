...
public class PdlDødsfallFagsakTilVurderingUtleder implements FagsakerTilVurderingUtleder {
    ...
    private HashMap<Fagsak, List<ÅrsakOgPerioder>> finnPåvirketFagsak(AktørId aktør, LocalDate aktuellDato, String hendelseId) {
        ...
        var fagsakForAktør = finnFagsakerForAktørTjeneste.hentRelevantFagsakForAktørSomSøker(FagsakYtelseType.AKTIVITETSPENGER, aktør, aktuellDato);
        if (fagsakForAktør.isPresent()) {
            if (erNyInformasjonIHendelsen(fagsakForAktør.get(), aktør, aktuellDato, hendelseId)) {
        ...
        finnFagsakerForAktørTjeneste.hentRelevantFagsakForAktørSomBarnAvSøker(FagsakYtelseType.AKTIVITETSPENGER, aktør, aktuellDato)
            .stream()

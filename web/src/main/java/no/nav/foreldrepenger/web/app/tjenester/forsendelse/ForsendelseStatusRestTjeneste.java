...
public class ForsendelseStatusRestTjeneste {
    ...
    @Operation(description = "Søker om status på prossesseringen av et mottatt dokument", tags = "dokumentforsendelse", responses = {
            @ApiResponse(responseCode = "200", description = "Status og Periode", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ForsendelseStatusDataDTO.class)))
    })
    public ForsendelseStatusDataDTO getStatusInformasjon(@NotNull @QueryParam("forsendelseId") @Parameter(description = "forsendelseId") @Valid @TilpassetAbacAttributt(supplierClass = AbacAttributtSupplier.class) ForsendelseIdDto forsendelseIdDto) {

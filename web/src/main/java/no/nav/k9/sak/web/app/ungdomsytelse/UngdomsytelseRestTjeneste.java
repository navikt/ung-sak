package no.nav.k9.sak.web.app.ungdomsytelse;
...
import no.nav.k9.sak.kontrakt.ungdomsytelse.beregning.UngdomsytelseSatsPeriodeDto;
import no.nav.k9.sak.kontrakt.ungdomsytelse.uttak.UngdomsytelseUttakPeriodeDto;
...
import no.nav.k9.sak.ytelse.ung.beregning.UngdomsytelseSatsPerioder;
    ...
    public List<UngdomsytelseSatsPeriodeDto> getUngdomsytelseInnvilgetSats(@NotNull @QueryParam(BehandlingUuidDto.NAME) @Parameter(description = BehandlingUuidDto.DESC) @Valid @TilpassetAbacAttributt(supplierClass = AbacAttributtSupplier.class) BehandlingUuidDto behandlingUuid) {
        ...
        if (perioder == null){
            ...
            return perioder.getPerioder().stream()
                .map(p->new UngdomsytelseSatsPeriodeDto(p.getPeriode().getFomDato(), p.getPeriode().getTomDato(), p.getDagsats(), p.getGrunnbeløpFaktor(), p.getGrunnbeløp()))
                .toList();
        }
    }

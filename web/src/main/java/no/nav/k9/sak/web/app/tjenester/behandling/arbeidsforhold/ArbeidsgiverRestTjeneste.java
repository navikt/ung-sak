    ...
    @Operation(description = "Henter informasjon om alle arbeidsgivere knyttet til bruker",
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "Returnerer ArbeidsgiverOversiktDto",
                content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ArbeidsgiverOgArbeidsforholdOversiktDto.class)))
        })
    @BeskyttetRessurs(action = READ, resource = FAGSAK)
    public ArbeidsgiverOgArbeidsforholdOversiktDto getArbeidsgiverOpplysninger(@NotNull @QueryParam(BehandlingUuidDto.NAME) @Parameter(description = BehandlingUuidDto.DESC) @TilpassetAbacAttributt(supplierClass = AbacAttributtSupplier.class) @Valid BehandlingUuidDto uuidDto) {
        UUID behandlingUuid = uuidDto.getBehandlingUuid();
        return arbeidsgiverOversiktTjeneste.getArbeidsgiverOgArbeidsforholdOpplysninger(behandlingUuid);
    }

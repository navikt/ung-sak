...
class UtledStatusForPerioderPåBehandlingTest {
    ...
    void skal_utlede_status_som_annullert_når_opphør_aldri_ble_iverksatt() {
        ...
        var periode = perioderMedÅrsak.get(0);
        assertThat(periode.getPeriode()).isEqualTo(new Periode(periodeTilVurdering.getFomDato(), periodeTilVurdering.getTomDato()));
        assertThat(periode.getÅrsaker()).isEqualTo(Set.of(ÅrsakTilVurdering.OPPHØR_ANNULERT_UNGDOMSPROGRAM));
    }
    ...
    void skal_vise_annullert_når_opphør_og_opphevelse_slås_sammen_uten_at_opphøret_ble_iverksatt() {
        ...
        assertThat(unikeÅrsaker)
            .contains(ÅrsakTilVurdering.OPPHØR_ANNULERT_UNGDOMSPROGRAM)

...
public class NulledePerioderTest {
    ...
    public void skal_håndtere_overlapp_mellom_arbeidsgivere() {
        ...
        var oppgittFraværPerioder = Set.of(new OppgittFraværPeriode(LocalDate.now().minusWeeks(6), LocalDate.now().minusDays(5), UttakArbeidType.ARBEIDSTAKER, Duration.ZERO),
            new OppgittFraværPeriode(LocalDate.now().minusDays(5), LocalDate.now(), UttakArbeidType.ARBEIDSTAKER, Duration.ZERO));

        var vilkårsPerioderFraSøktePerioder = søktePerioder.utledPeriodeFraSøknadsPerioder(oppgittFraværPerioder, List.of());
    ...
    public void skal_håndtere_overlapp_mellom_arbeidsgivere_på_tvers_av_behandlinger() {
        var søktePerioder = new NulledePerioder(null);
        var oppgittFraværPerioder = Set.of(new OppgittFraværPeriode(LocalDate.now().minusDays(6), LocalDate.now().minusDays(6), UttakArbeidType.ARBEIDSTAKER, Duration.ZERO),
            new OppgittFraværPeriode(LocalDate.now().minusDays(4), LocalDate.now().minusDays(4), UttakArbeidType.ARBEIDSTAKER, Duration.ZERO));

        var vilkårsPerioderFraSøktePerioder = søktePerioder.utledPeriodeFraSøknadsPerioder(oppgittFraværPerioder, List.of(new OppgittFraværPeriode(LocalDate.now().minusWeeks(6), LocalDate.now().minusDays(5), UttakArbeidType.ARBEIDSTAKER, Duration.ofHours(2))));

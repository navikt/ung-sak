...
class FraværPeriodeSammenslåerTest {
    LocalDate måndag = LocalDate.of(2021, 04, 26);

    Periode mån = new Periode(måndag, måndag);
    Periode tis = new Periode(måndag.plusDays(1), måndag.plusDays(1));
    Periode ons = new Periode(måndag.plusDays(2), måndag.plusDays(2));
    Periode lør = new Periode(måndag.plusDays(5), måndag.plusDays(5));
    ...
    void skal_slå_sammen_perioder_som_ligger_inntil_hverandre_og_har_samme_innhold() {
        ...
        assertThat(sammenslåttPeriode.getPeriode()).isEqualTo(new Periode(måndag, måndag.plusDays(2)));
    ...
    void fjerner_helger_fra_perioder() {
        LocalDate fom = måndag;
        LocalDate tom = måndag.plusDays(8);
        var resultat = fjernHelgdager(new Periode(fom, tom));

        Assertions.assertTrue(resultat.size() == 2);

        Boolean inneholderHelgdag = false;
        for(Periode periode : resultat) {
            var dato = periode.getFraOgMed();
            while(dato.isBefore(periode.getTilOgMed().plusDays(1L))) {
                if(dato.getDayOfWeek().equals(DayOfWeek.SATURDAY) || fom.getDayOfWeek().equals(DayOfWeek.SUNDAY)) {
                    inneholderHelgdag = true;
                }
                dato = dato.plusDays(1L);
            }
        }

        Assertions.assertFalse(inneholderHelgdag);
    }
}

...
class VilkårsavklaringGrunnlagRepositoryTest {
    ...
    @Test
    void skal_kopiere_ferdigstilte_men_ikke_foreslaatte_avklaringer_til_ny_behandling() {
        var foreslått = lagAvklaring(FOM, TOM);
        repository.lagreForeslåtteAvklaringer(behandling.getId(), VILKÅR_TYPE, Set.of(foreslått));
        repository.ferdigstillForeslåtteAvklaringer(behandling.getId(), VILKÅR_TYPE);
        ...
        // Forslaget (nå også ferdigstilt) skal fortsatt ligge uendret på den opprinnelige behandlingen
        var gammeltGrunnlag = repository.hentGrunnlagHvisEksisterer(behandling.getId(), VILKÅR_TYPE).orElseThrow();
        assertThat(gammeltGrunnlag.getForeslåtteAvklaringer())
            .extracting(VilkårPeriodeAvklaring::getReferanse)
            .containsExactly(foreslått.getReferanse());
        assertThat(gammeltGrunnlag.getFerdigstilteAvklaringer())
            .extracting(VilkårPeriodeAvklaring::getReferanse)
            .containsExactly(foreslått.getReferanse());
    }

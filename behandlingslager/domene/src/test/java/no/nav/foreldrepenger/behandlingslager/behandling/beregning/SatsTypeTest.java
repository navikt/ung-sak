...
public class SatsTypeTest {
    ...
    public void skal_teste_verdier_for_sats_gbeløp_og_gsnitt() {
        List<BeregningSats> satser = repository.hentAlle(BeregningSats.class);
        List<BeregningSats> grunnbeløpListe = satser.stream().filter(sats -> sats.getSatsType().equals(BeregningSatsType.GRUNNBELØP)).collect(Collectors.toList());
        List<BeregningSats> gsnittListe = satser.stream().filter(sats -> sats.getSatsType().equals(BeregningSatsType.GSNITT)).collect(Collectors.toList());

 ...
 */
public class AktivitetspengerTestScenarioBuilder {

    public static final FagsakYtelseType TEST_YTELSE = FagsakYtelseType.AKTIVITETSPENGER;
    ...
    private AktivitetspengerTestScenarioBuilder() {
        this.fagsakBuilder = FagsakBuilder
            .nyFagsak(TEST_YTELSE)
            .medSaksnummer(new Saksnummer(nyId() + ""));
        settDefaultSøknad();
    }

    private AktivitetspengerTestScenarioBuilder(AktørId aktørId) {
        this.fagsakBuilder = FagsakBuilder
            .nyFagsak(TEST_YTELSE)
            .medSaksnummer(new Saksnummer(nyId() + ""))
            .medBruker(aktørId);
    ...
    public static AktivitetspengerTestScenarioBuilder builderUtenSøknad() {
        var scenario = new AktivitetspengerTestScenarioBuilder();
        scenario.utenSøknad();
        return scenario;
    }

    public static AktivitetspengerTestScenarioBuilder builderUtenSøknad(AktørId aktørId) {
        var scenario = new AktivitetspengerTestScenarioBuilder(aktørId);
        scenario.utenSøknad();
        return scenario;
    }

    public static AktivitetspengerTestScenarioBuilder builderMedSøknad() {
        return new AktivitetspengerTestScenarioBuilder();
    }

    public static AktivitetspengerTestScenarioBuilder builderMedSøknad(AktørId aktørId) {
        return new AktivitetspengerTestScenarioBuilder(aktørId);
    }
    ...
    public BehandlingRepository mockBehandlingRepository() {
        ...
        when(behandlingRepository.hentBehandling(Mockito.any(UUID.class))).thenAnswer(a -> {
            throw new org.jboss.weld.exceptions.UnsupportedOperationException("Ikke implementert for AbstractTestScenario");
        });
    ...
    public AktivitetspengerTestScenarioBuilder medAktivitetspengerTestGrunnlag(AktivitetspengerTestScenario aktivitetspengerTestscenario) {
        this.aktivitetspengerTestscenario = aktivitetspengerTestscenario;
        return this;
    }

    public AktivitetspengerTestScenario getAktivitetspengerTestscenario() {
        return aktivitetspengerTestscenario;
    }

    public Behandling buildOgLagreMedAktivitspenger(AktivitetspengerTestRepositories repositories) {
        settOppVilkårOgPersoner();

        build(repositories.repositoryProvider());

        //Ung ting
        buildAktivitetspenger(repositories, behandling);
        return behandling;
    }

    public Behandling buildOgLagreNyAktivitetspengerBehandlingPåEksisterendeSak(AktivitetspengerTestRepositories repositories) {
        settOppVilkårOgPersoner();
        Behandling nyBehandling = buildBehandling(repositories.repositoryProvider());
        buildAktivitetspenger(repositories, nyBehandling);
        return nyBehandling;
    }

    private void settOppVilkårOgPersoner() {
        if (aktivitetspengerTestscenario == null)
    ...
    private void settOppPersoner() {
        if (personer == null) {
            ...
            Personas ungdomPersonas = opprettBuilderForRegisteropplysninger()
                .medPersonas()
                .ungdom(ungdom, aktivitetspengerTestscenario.fødselsdato(), aktivitetspengerTestscenario.navn(), aktivitetspengerTestscenario.dødsdato());

            aktivitetspengerTestscenario.barn().forEach(it -> {
    ...
    private void buildAktivitetspenger(AktivitetspengerTestRepositories repositories, Behandling behandling1) {
        ...
        if (aktivitetspengerTestscenario.tilkjentYtelsePerioder() != null) {
            repositories.tilkjentYtelseRepository().lagre(behandling1.getId(), aktivitetspengerTestscenario.tilkjentYtelsePerioder(), "input", "sporing");
        }
    ...
    public AktivitetspengerTestScenarioBuilder medSøknadDato(LocalDate søknadsdato) {
    ...
    private Behandling.Builder grunnBuild(BehandlingRepositoryProvider repositoryProvider) {
        ...
        } else if (aktivitetspengerTestscenario != null && !aktivitetspengerTestscenario.behandlingTriggere().isEmpty()) {
            var årsakerFraTriggere = aktivitetspengerTestscenario.behandlingTriggere().stream()
    ...
    private DatoIntervallEntitet utledFagsakPeriode() {
        if (aktivitetspengerTestscenario != null) {
            LocalDateTimeline<Boolean> søktTidslinje = new LocalDateTimeline<>(aktivitetspengerTestscenario.søknadsperioder().stream().map(it -> new LocalDateSegment<>(it.getFom(), it.getTom(), true)).toList(), StandardCombinators::alwaysTrueForMatch);
            return DatoIntervallEntitet.fraOgMedTilOgMed(søktTidslinje.getMinLocalDate(), søktTidslinje.getMaxLocalDate());
        }
    ...
    public AktivitetspengerTestScenarioBuilder medSaksnummer(Saksnummer saksnummer) {
        fagsakBuilder.medSaksnummer(saksnummer);
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medFagsakId(Long id) {
        this.fagsakId = id;
        return this;
    }
    ...
    public AktivitetspengerTestScenarioBuilder medBehandlingStatus(BehandlingStatus status) {
        this.behandlingStatus = Objects.requireNonNull(status, "status");
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medBehandlingÅrsak(BehandlingÅrsakType behandlingÅrsakType) {
        this.behandlingÅrsakType = Objects.requireNonNull(behandlingÅrsakType, "behandlingÅrsakType");
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medBehandlingsresultat(BehandlingResultatType behandlingResultatType) {
    ...
    public AktivitetspengerTestScenarioBuilder leggTilVilkår(VilkårType vilkårType, Utfall utfall, Periode periode) {
    ...
    public AktivitetspengerTestScenarioBuilder medBruker(AktørId aktørId) {
        fagsakBuilder
            .medBruker(aktørId);
    ...
    public AktivitetspengerTestScenarioBuilder medBehandlingStegStart(BehandlingStegType startSteg) {
        this.startSteg = startSteg;
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medBehandlingstidFrist(LocalDate behandlingstidFrist) {
        this.behandlingstidFrist = behandlingstidFrist;
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medBehandlendeEnhet(String behandlendeEnhet) {
        this.behandlendeEnhet = behandlendeEnhet;
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medOpplysningerOppdatertTidspunkt(LocalDateTime opplysningerOppdatertTidspunkt) {
        this.opplysningerOppdatertTidspunkt = opplysningerOppdatertTidspunkt;
        return this;
    }

    public AktivitetspengerTestScenarioBuilder medRegisterOpplysninger(PersonInformasjon personinfo) {
    ...
    public AktivitetspengerTestScenarioBuilder medOriginalBehandling(Behandling originalBehandling, BehandlingÅrsakType behandlingÅrsakType) {
        return medOriginalBehandling(originalBehandling, behandlingÅrsakType, false);
    }

    public AktivitetspengerTestScenarioBuilder medManuellOpprettet() {
        manueltOpprettet = true;
        return this;
    }

    private AktivitetspengerTestScenarioBuilder medOriginalBehandling(Behandling originalBehandling, BehandlingÅrsakType behandlingÅrsakType, boolean manueltOpprettet) {

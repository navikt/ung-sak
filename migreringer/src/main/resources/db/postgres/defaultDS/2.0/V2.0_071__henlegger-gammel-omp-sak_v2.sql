...
where fagsak_id IN (
  ...
  AND cast(behandling_id as bigint) IN (
    select b.id from behandling b where b.fagsak_id IN (select f.id from fagsak f where f.saksnummer IN ('8DSJM')) AND  b.behandling_status NOT IN ('AVSLU')
);
...
with saker as (select b.fagsak_id, f.saksnummer, b.id as behandling_id
               ...
               where b.behandling_status NOT IN ('AVSLU') and f.saksnummer IN ('8DSJM'))

...
update psb_unntak_etablert_tilsyn_periode it set vurdert_av = (select opprettet_av from psb_unntak_etablert_tilsyn_periode where fom = it.fom and tom = it.tom and begrunnelse is not distinct from it.begrunnelse and resultat = it.resultat and soeker_aktoer_id = it.soeker_aktoer_id and kilde_behandling_id = it.kilde_behandling_id order by opprettet_tid limit 1),
                                                 vurdert_tid = (select opprettet_tid from psb_unntak_etablert_tilsyn_periode where fom = it.fom and tom = it.tom and begrunnelse is not distinct from it.begrunnelse and resultat = it.resultat and soeker_aktoer_id = it.soeker_aktoer_id and kilde_behandling_id = it.kilde_behandling_id order by opprettet_tid limit 1);

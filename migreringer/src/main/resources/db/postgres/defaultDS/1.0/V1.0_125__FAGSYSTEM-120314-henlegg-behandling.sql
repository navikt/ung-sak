...
where fagsak_id IN (select f.id from fagsak f where f.saksnummer IN ('631SW')) AND  behandling_status NOT IN ('AVSLU', 'IVED');

-- fjern gamle tasks
update prosess_task set status='KJOERT', blokkert_av=NULL where id in (
    select prosess_task_id from fagsak_prosess_task fpt inner join fagsak f on f.id=fpt.fagsak_id and f.saksnummer IN ('631SW'))
    and status NOT IN ('KJOERT', 'FERDIG');

delete from fagsak_prosess_task 
where fagsak_id IN (
 select f.id from  fagsak f where f.saksnummer IN ('631SW')
) 
AND cast(behandling_id as bigint) IN (
  select b.id from behandling b where b.fagsak_id IN (select f.id from fagsak f where f.saksnummer IN ('631SW')) AND  b.behandling_status NOT IN ('AVSLU', 'IVED')
);
...
with saker as (select b.fagsak_id, f.saksnummer, b.id as behandling_id
 ...
 where b.behandling_status NOT IN ('AVSLU', 'IVED') and f.saksnummer IN ('631SW'))
...
'callId=CallId_FAGSYSTEM-120314-' || saksnummer || '

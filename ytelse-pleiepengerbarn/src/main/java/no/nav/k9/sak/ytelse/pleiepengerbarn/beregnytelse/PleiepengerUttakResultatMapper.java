...
@ApplicationScoped
public class PleiepengerUttakResultatMapper implements no.nav.k9.sak.ytelse.beregning.UttakResultatMapper {

    PleiepengerUttakResultatMapper() {
        // for proxy
    }
    ...
    private static List<UttakAktivitet> toUttakAktiviteter(InnvilgetUttaksplanperiode uttaksplanperiode) {
        return uttaksplanperiode.getUtbetalingsgrader().stream()
            .map(PleiepengerUttakResultatMapper::mapTilUttaksAktiviteter)

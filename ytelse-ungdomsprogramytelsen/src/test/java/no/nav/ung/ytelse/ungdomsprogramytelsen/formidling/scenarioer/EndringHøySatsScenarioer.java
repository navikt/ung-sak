...
public class EndringHøySatsScenarioer {
    ...
    public static UngTestScenario endring25År(LocalDate fødselsdato) {
        ...
        var satser = new LocalDateTimeline<>(List.of(
            new LocalDateSegment<>(fagsakPeriode.getFomDato(), tjuvefemårsdag.minusDays(1), BrevScenarioerUtils.lavSatsBuilder(fom).build()),
            new LocalDateSegment<>(tjuvefemårsdag, fagsakPeriode.getTomDato(), BrevScenarioerUtils.høySatsBuilder(fom).build())
        ));

        var programPerioder = List.of(new UngdomsprogramPeriode(fagsakPeriode.getFomDato(), LocalDateInterval.TIDENES_ENDE));
        ...
        return new UngTestScenario(
            ...
            new LocalDateTimeline<>(fagsakPeriode, Utfall.OPPFYLT),
            fødselsdato,
            List.of(fagsakPeriode.getFomDato()),
            Set.of(new Trigger(BehandlingÅrsakType.RE_TRIGGER_BEREGNING_HØY_SATS, DatoIntervallEntitet.fra(tjuvefemårsdag, fagsakPeriode.getTomDato()))), Collections.emptyList(),
    ...
    public static UngTestScenario endring25ÅrMedToBarn(LocalDate fødselsdato) {
        ...
        var satser = new LocalDateTimeline<>(List.of(
            new LocalDateSegment<>(fagsakPeriode.getFomDato(), tjuvefemårsdag.minusDays(1), BrevScenarioerUtils.lavSatsMedBarnBuilder(fom, 2).build()),
            new LocalDateSegment<>(tjuvefemårsdag, fagsakPeriode.getTomDato(), BrevScenarioerUtils.høySatsBuilderMedBarn(tjuvefemårsdag, 2).build())
        ));
        ...
        return new UngTestScenario(
            ...
            fødselsdato,
            List.of(fagsakPeriode.getFomDato()),
            Set.of(new Trigger(BehandlingÅrsakType.RE_TRIGGER_BEREGNING_HØY_SATS, DatoIntervallEntitet.fra(tjuvefemårsdag, fagsakPeriode.getTomDato()))),
                List.of(
